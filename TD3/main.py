...
class ActorCritic(tf.keras.Model):
    def __init__(self, hidden_sizes=(300,), activation=tf.nn.relu,
        ...
        self.net_q1 = NetBlock(list(hidden_sizes)+[1], activation, None)
        self.net_q2 = NetBlock(list(hidden_sizes)+[1], activation, None)
        # target actor and critic
        self.tar_net_pi = NetBlock(list(hidden_sizes)+[self.act_dim], activation, output_activation)
        self.tar_net_q1 = NetBlock(list(hidden_sizes)+[1], activation, None)
        self.tar_net_q2 = NetBlock(list(hidden_sizes)+[1], activation, None)
    ...
    def get_pi_q(self, s, a, net="main"):
        ...
        if net == 'main':
            pi_for_compute, q1_for_compute, q2_for_compute = self.net_pi, self.net_q1, self.net_q2
        elif net == 'target':
            pi_for_compute, q1_for_compute, q2_for_compute = self.tar_net_pi, self.tar_net_q1, self.tar_net_q2
        else:
            raise Exception("Invalid net!")

        if a is None:
            a = tf.convert_to_tensor(np.random.random((s.shape[0], self.act_dim)), tf.float32)
        pi = pi_for_compute(s) * self.act_limit
        q1 = tf.squeeze(q1_for_compute(tf.concat([s, a], axis=-1)))
        q2 = tf.squeeze(q2_for_compute(tf.concat([s, a], axis=-1)))
        q_pi = tf.squeeze(q1_for_compute(tf.concat([s, pi], axis=-1)))
        return pi, q1, q2, q_pi

    def get_target(self, r, s_next, done, gamma=0.99, target_noise=0.2, noise_clip=0.5):
        # target policy smoothing, by adding noise to target action
        pi_targ, _, _, _ = self.get_pi_q(s_next, None, net='target')
        epsilon = tf.random_normal(tf.shape(pi_targ), stddev=target_noise)
        epsilon = tf.clip_by_value(epsilon, -noise_clip, noise_clip)
        a_next = pi_targ + epsilon
        a_next = tf.clip_by_value(a_next, -1.*self.act_limit, self.act_limit)
        # compute the target Q value of q1 and q2
        _, q1_targ, q2_targ, _ = self.get_pi_q(s_next, a_next, net='target')

        # twin q value
        min_q_targ = tf.minimum(q1_targ, q2_targ)
        return r + gamma * (1-done) * min_q_targ

    def cal_loss(self, s, a, r, s_next, done):
        # pi loss
        pi, q1, q2, q1_pi = self.get_pi_q(s, a, net='main')
        pi_loss = -tf.reduce_mean(q1_pi)
        # Q-loss of two value network
        target = self.get_target(r, s_next, done)
        q1_loss = tf.reduce_mean((q1 - tf.stop_gradient(target)) ** 2)
        q2_loss = tf.reduce_mean((q2 - tf.stop_gradient(target)) ** 2)
        return pi_loss, q1_loss, q2_loss

    def target_init(self):
        self.tar_net_pi.set_weights(self.net_pi.get_weights())
        self.tar_net_q1.set_weights(self.net_q1.get_weights())
        self.tar_net_q2.set_weights(self.net_q2.get_weights())
    ...
    def target_update(self, polyak=0.995):
        ...
        self.tar_net_q1.set_weights([polyak * w1 + (1. - polyak) * w2 for w1, w2 in
                    zip(self.tar_net_q1.get_weights(), self.net_q1.get_weights())])
        self.tar_net_q2.set_weights([polyak * w2 + (1. - polyak) * w2 for w1, w2 in
                    zip(self.tar_net_q2.get_weights(), self.net_q2.get_weights())])

    def call(self, x):
        pass


def td3(env_name, steps_per_epoch=5000, epochs=500, pi_lr=1e-3, q_lr=1e-3,
        batch_size=100, start_steps=10000, max_ep_len=1000, save_freq=10, policy_delay=2):
    """ Input:
        ...
        # Parameters where TD3 different from DDPG: target_noise, noise_clip, policy_delay
    """
    ...
    q1_optimizer = tf.train.AdamOptimizer(learning_rate=q_lr)
    q2_optimizer = tf.train.AdamOptimizer(learning_rate=q_lr)
    ...
    for t in range(total_steps):
        ...
        if d or (ep_len == max_ep_len):
            ...
            for j in range(ep_len):         # 训练 1000 个时间步
                batch = replay_buffer.sample_batch(batch_size)
                with tf.GradientTape(persistent=True) as tape:
                    pi_loss, q1_loss, q2_loss = actor_critic.cal_loss(
                        s=batch['obs1'], a=batch['acts'], r=batch['rews'],
                        s_next=batch['obs2'], done=batch['done'])
                gradients_q1 = tape.gradient(q1_loss, actor_critic.net_q1.trainable_variables)
                gradients_q2 = tape.gradient(q2_loss, actor_critic.net_q2.trainable_variables)
                q1_optimizer.apply_gradients(zip(gradients_q1, actor_critic.net_q1.trainable_variables))
                q2_optimizer.apply_gradients(zip(gradients_q2, actor_critic.net_q2.trainable_variables))
            ...
            print("Pi loss:", pi_loss.numpy(), "Q1 loss:", q1_loss.numpy(), "Q2 loss:", q2_loss.numpy())
...
if __name__ == '__main__':
    ...
    # actor_critic = ActorCritic(action_space=ac_space)
    # pi_loss, q1_loss, q2_loss = actor_critic.cal_loss(s=s, a=a, r=r, s_next=s_next, done=done)
    # print("OUT:", pi_loss.numpy(), q1_loss.numpy(), q2_loss.numpy())

    # td3(env_name="HalfCheetah-v2")
    play(model_path="model/model_30.h5")
